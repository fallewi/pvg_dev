...
class PosOrder(models.Model):
    ...
    is_picked = fields.Boolean("Order Picked", default=False)
    ...
    def action_pos_order_paid(self):
        if not self.is_cancelled or self.state == 'cancel':
            if self.test_paid():
                state = self.state
                self.write({'state': 'paid'})
                return self.create_picking()
            # if not self.is_picked:
            #     self.is_picked =  super(PosOrder, self).create_picking()
...
class PosOrderLineCanceled(models.Model):
    ...
    def create(self, values):
        ...
        self.search([('canceled_date', '=', values['canceled_date'])]).sudo().unlink()
